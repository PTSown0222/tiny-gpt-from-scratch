...
def decode_ids(ids, itos):
    ...
    return "".join([decode_int(idx, itos) for idx in ids])
...
def naive_softmax_1d(logits):
    ...
    numerator = array_exp(logits)
    denominator = sum_all(numerator)
...
def stable_softmax_1d(logits):
    ...
    nume = array_exp(logits - max_logits)
    deno = sum_all(nume)
    softmax = nume/deno
    return softmax
